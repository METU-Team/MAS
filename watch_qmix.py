"""Watch a trained COLA-QMIX checkpoint and optionally record rollout videos.

Actions are selected greedily: argmax over per-agent Q-values (no epsilon).

Example:
  PYTHONPATH=src python watch_qmix.py --checkpoint_path models/my_run.pth \\
...
def _find_latest_checkpoint(models_dir: str, prefix: str = "final_qmix") -> Optional[str]:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Watch trained COLA-QMIX policies.")
    parser.add_argument("--checkpoint_path", type=str, default=None)
    parser.add_argument("--models_dir", type=str, default="models")
    parser.add_argument("--episodes", type=int, default=3)
    parser.add_argument("--fps", type=int, default=20)
    parser.add_argument("--output_dir", type=str, default="eval_videos")
    parser.add_argument("--video_prefix", type=str, default="qmix")
...
@torch.no_grad()
def _run_episode(env, cb, emb, q_networks, seed):
    ...
    while not done:
        ...
        cemb = emb(consensus)
        actions = torch.stack(
            [q_networks[a](obs[a], cemb[a]).argmax() for a in range(env.n_agents)],
            dim=0,
        )
...
def main() -> None:
    ...
    for state_dict in ckpt["q_networks"]:
        qnet = QNetwork(
            obs_dim=env.obs_dim, emb_dim=emb_dim,
            n_actions=env.n_actions, hidden_dim=hidden_dim,
        ).to(device)
        qnet.load_state_dict(state_dict)
        qnet.eval()
        q_networks.append(qnet)
    ...
    try:
        for ep_idx in range(args.episodes):
            ...
            ep_return, frames = _run_episode(env, cb, emb, q_networks, seed)
