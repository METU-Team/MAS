"""History-aware MADDPG update logic.

This updater keeps the same optimizer/update structure as the classic path,
but consumes sequence fields for consensus-builder inputs.
"""
...
class HistoryAwareMADDPGUpdater(UpdateModule):
    """Coordinates one full training update for history-aware COLA + MADDPG."""
    ...
    def update(self, batch: Dict[str, torch.Tensor]) -> Dict[str, object]:
        ...
        batch_size, n_agents, _ = actions.shape

        # 1) Consensus builder update using sequence inputs.
        loss_cb, consensus = self.consensus_builder(obs_seq)
        ...
        with torch.no_grad():
            next_consensus = torch.stack(
                [self.consensus_builder.infer(next_obs_seq[b]) for b in range(batch_size)],
                dim=0,
            )
